...
for (var i=1;i<11;i++){
    ...
    const rule = require("./week7/replace_rules.json"); // to introduce the configuration file
    ...
    var locationList = [];
        ...
        $('tbody tbody tbody tr').each(function(i, elem){
            ...
            // Meeting Name
            var meetingName = $(elem).find('b').eq(0).text().split('- ')[0].trim();
            
            // Street Address
            var _address = $(elem).find('b')[0].nextSibling.nextSibling // to navigate our target parsing location. the same way as before
            var address_string = $(_address).text().split(',')[0].split(/- |Rm|Meeting|\(Red/)[0].trim() // to roughly get the address with some inconsistent spelled words
            ...
            // For an exception in Zone 7. If you want, just disable this block when parsing other zones 
            if (meetingID == '1529'){ 
